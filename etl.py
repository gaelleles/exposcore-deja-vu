...
MATERIALS_FILEPATH = [
    ...
    {"material": "Adhésif mat pour découpe", "filepath": "3225 - Adhésif mat pour découpe /critères-adhésif-expr.xlsx"},
...
def read_material_sheets(filepath, material_name: str):
    ...
    file_bytes_or_path = download_file("https://nuage.relief-aura.fr/s/axt3qBkEsFWcDpr", filepath)
    ...
    if eco_sheet:
        ...
        df_eco.columns = ['Catégorie', 'Description de la catégorie', 'Valeur', 'Unité', 'Commentaire', 'Source', 'Année']
        ...
        warnings.warn(f"Aucune feuille 'écologique pour comparaison' trouvée dans le fichier Excel. Fichier : {filepath}. Feuilles disponibles : {sheet_names}")
        df_eco = None
    
    if social_sheet:
        social_sheet_name = social_sheet[0]
        df_social = file.parse(sheet_name=social_sheet_name, skiprows=1)
        df_social.columns = ['Catégorie', 'Typologie', 'Critère', 'Unité', 'Description', 'Données', 'Sources', 'Année']
        df_social.insert(0, "Matériau", material_name)
    else:
        raise ValueError(f"Aucune feuille 'Impact sociaux' trouvée dans le fichier Excel. Fichier : {filepath}. Feuilles disponibles : {sheet_names}")
    
    if usage_sheet:
        usage_sheet_name = usage_sheet[0]
        df_usage = file.parse(sheet_name=usage_sheet_name, skiprows=1)
        df_usage.columns = ['Catégorie', 'Critère', 'Unité', 'Description', 'Données', 'Sources', 'Année']
        df_usage.insert(0, "Matériau", material_name)
    else:
        raise ValueError(f"Aucune feuille 'Usage' trouvée dans le fichier Excel. Fichier : {filepath}. Feuilles disponibles : {sheet_names}")
...
def load_eco_df_to_db(df_eco, conn):
    ...
    for _, row in df_eco.iterrows():
        ...
        """, (
            ...
            str(row["Description de la catégorie"]) if pd.notna(row["Description de la catégorie"]) else None,
...
def get_materials(db_path=DB_PATH):
    conn = init_db(db_path)
    materials = pd.read_sql("SELECT DISTINCT materiau FROM impact_eco ORDER BY materiau", conn)["materiau"].tolist()
    conn.close()
    return materials
